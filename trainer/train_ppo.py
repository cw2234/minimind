    ...
    with torch.no_grad():
        ...
        for i, (prompt, response) in enumerate(zip(prompts, responses)):
            ...
            if "</think>" in response:
                ...
                rewards[i] += (
                    1.0 if 20 <= len(thinking_content.strip()) <= 300 else -0.5
                )
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        if args.debug_mode and is_main_process() and step % args.debug_interval == 0:
            for i in range(len(prompts)):
                ...
                Logger(
                    f"[DEBUG] prompt_len={prompt_lens[i].item()}, response_len={len(responses_text[i])}"
                )
        ...
        resp_lengths = (
            torch.where(has_eos, eos_pos + 1, resp_lengths).long().clamp(min=1)
        )
        resp_policy_mask = (
            (resp_idx < resp_lengths.unsqueeze(1)) & resp_pad_mask
        ).float()
        resp_value_mask = resp_policy_mask.clone()

        with (
            torch.no_grad()
        ):  # Rollout阶段只需推理获取old_logp和old_values，切断梯度省显存
            critic_for_rollout = (
                critic_model.module
            ...
            ref_resp_logp = (
                F.log_softmax(
                    ref_model(input_ids=gen_out, attention_mask=full_mask).logits[
                        :, :-1
                    ],
                    dim=-1,
                )
            ...
            token_rewards[
                torch.arange(B, device=args.device)[valid_resp], last_idx[valid_resp]
            ] += rewards[valid_resp]  # 末尾加外部奖励
            ...
            adv_mean = (
                advantages * resp_policy_mask
            ).sum() / resp_policy_mask.sum().clamp(min=1)
            adv_var = (
                (advantages - adv_mean) ** 2 * resp_policy_mask
            ).sum() / resp_policy_mask.sum().clamp(min=1)
            advantages = (
                (advantages - adv_mean) * torch.rsqrt(adv_var + 1e-8) * resp_policy_mask
            )
        ...
        for ppo_epoch in range(args.ppo_update_iters):
            ...
            for i in range(0, B, mb_size):
                ...
                mb_values_seq = critic_unwrapped(
                    input_ids=gen_out[inds], attention_mask=full_mask[inds]
                )
                ...
                policy_loss = (
                    ...
                    * resp_policy_mask[inds]
                ).sum() / resp_policy_mask[inds].sum().clamp(
                    min=1
                ...
                value_loss = (
                    ...
                    * (
                    ...
                    / resp_value_mask[inds].sum().clamp(min=1)
                )
                ...
                if stop_ppo:
                    ...
                    loss = (
                        policy_loss + args.vf_coef * value_loss + aux_loss
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--critic_learning_rate", type=float, default=5e-7, help="Critic学习率"
    )
    parser.add_argument(
        "--device",
        type=str,
    ...
    parser.add_argument(
        "--data_path", type=str, default="../dataset/rlaif.jsonl", help="RLAIF数据路径"
    )
    ...
    parser.add_argument(
        "--cliprange_value", type=float, default=0.2, help="Value function裁剪范围"
    )
    parser.add_argument(
        "--ppo_update_iters", type=int, default=2, help="同一批rollout重复更新次数"
    )
    parser.add_argument(
        "--early_stop_kl", type=float, default=0.25, help="PPO early stop 的 KL 阈值"
    )
    parser.add_argument(
        "--mini_batch_size", type=int, default=2, help="PPO每次更新的minibatch大小"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--debug_mode", action="store_true", help="是否打印训练调试采样"
    )
    parser.add_argument(
        "--debug_interval",
        type=int,
        default=20,
        help="debug模式下每隔多少step打印一次采样",
    )
    parser.add_argument(
        "--thinking_ratio",
        type=float,
