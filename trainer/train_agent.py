...
MOCK_RESULTS = {
    ...
    "unit_converter": lambda args: {
        "result": round(
            ...
            * UNIT_DATA.get(
                f"{args.get('from_unit', '').lower()}_{args.get('to_unit', '').lower()}",
                1,
            ),
            4,
        )
    ...
    "get_current_time": lambda args: {
        "datetime": TIME_DATA.get(
            args.get("timezone", "Asia/Shanghai"), "2025-03-07 14:30:00"
        ),
    ...
    "get_exchange_rate": lambda args: {
        ...
        "rate": EXCHANGE_DATA.get(
            (args.get("from_currency"), args.get("to_currency")), 1.0
        ),
    },
    "translate_text": lambda args: {
        "translated_text": TRANSLATE_DATA.get(
            (args.get("text"), args.get("target_language")), args.get("text", "")
        )
    },
}
...
CHECK_ARGS = {
    ...
    "unit_converter": lambda a: (
        a.get("value") is not None and a.get("from_unit") and a.get("to_unit")
    ),
    "get_current_weather": lambda a: bool(a.get("location")),
    "get_current_time": lambda a: True,
    "get_exchange_rate": lambda a: (
        bool(a.get("from_currency")) and bool(a.get("to_currency"))
    ),
...
def rollout_single(
    ...
    for turn in range(max_turns):
        ...
        inputs = tokenizer(context, return_tensors="pt", add_special_tokens=False).to(
            device
        )
        ...
        pairs = [
            (t, lp)
            for t, lp in zip(new_ids, new_logps)
...
def validate_gt_in_text(text, gt_list):
    ...
    nums = [
        float(x) for x in re.findall(r"(?<![\w.])[-+]?\d+(?:\.\d+)?(?![\w.])", text_num)
    ]
    return {
        g
        for g in gt_list
...
def calculate_rewards(
    ...
    for idx, response in enumerate(completions):
        ...
        turn_outputs = (
            turn_outputs_batch[idx] if turn_outputs_batch is not None else [response]
        )
        ...
        # -------- 有工具调用：执行结果奖励 --------
            ...
            for tool_call in tool_calls:
                ...
                valid_call_count += int(
                    bool(name in valid_names and check and check(raw))
                )
            tool_gap = abs(valid_call_count - len(gt)) + max(
                0, len(tool_calls) - valid_call_count
            )  # tool数差值
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        prompts = [
            tokenizer.apply_chat_template(
                m, tokenize=False, add_generation_prompt=True, tools=t
            )
            for m, t in zip(messages_batch, tools_batch)
        ]
        packed_samples = []
        for p, r, m, old_lp in zip(
            prompt_ids_batch,
            response_ids_batch,
            response_masks_batch,
            response_old_logps_batch,
        ...
        seq_lens = torch.tensor(
            [len(ids) for ids, _, _, _ in packed_samples], device=args.device
        )
        ...
        prompt_lens = torch.tensor(
            [prompt_len for _, _, prompt_len, _ in packed_samples], device=args.device
        )
        ...
        if args.debug_mode and is_main_process() and step % args.debug_interval == 0:
            for i in range(len(messages_batch)):
                ...
                for j in range(args.num_generations):
                    ...
                    Logger(
                        f"{'=' * 28} [DEBUG] gen[{i}][{j}] COMPLETION_BEGIN [{plen}:{slen}] {'=' * 28}"
                    )
                    Logger(text)
                    Logger(
                        f"{'=' * 29} [DEBUG] gen[{i}][{j}] COMPLETION_END {'=' * 29}"
                    )
        ...
        policy_loss = (
            (
                ...
                / token_counts[valid_rows].clamp(min=1)
            ).mean()
            if valid_rows.any()
        ...
        if step % args.log_interval == 0 or step == iters:
            ...
            ).sum().item() / max(token_counts.sum().item(), 1)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dtype", type=str, default="bfloat16", help="数据类型 bfloat16/float16"
    )
    ...
    parser.add_argument(
        "--use_moe", default=0, type=int, choices=[0, 1], help="是否使用MoE"
    )
    ...
    parser.add_argument(
        "--max_total_len", type=int, default=2500, help="训练侧最终总长度上界"
    )
    parser.add_argument(
        "--data_path",
        type=str,
        default="../dataset/agent_rl.jsonl",
        help="训练数据路径",
    )
    parser.add_argument(
        "--num_generations", type=int, default=4, help="每个prompt生成数量"
    )
    ...
    parser.add_argument(
        "--from_weight", default="full_sft", type=str, help="加载预训练权重名称"
    )
    parser.add_argument(
        "--from_resume",
        default=0,
        type=int,
        choices=[0, 1],
        help="是否从checkpoint恢复",
    )
    parser.add_argument("--use_wandb", action="store_true", help="是否使用wandb记录")
    parser.add_argument(
        "--wandb_project", type=str, default="MiniMind-Agent-RL", help="wandb项目名称"
    )
    parser.add_argument(
        "--use_compile",
        default=0,
    ...
    parser.add_argument(
        "--sglang_model_path", type=str, default="../model", help="SGLang tokenizer路径"
    )
    parser.add_argument(
        "--sglang_shared_path",
        type=str,
        default="./sglang_ckpt_agent",
        help="SGLang共享存储路径",
    )
    ...
    if args.use_wandb and is_main_process():
        ...
        wandb.init(
            ...
            name=f"Agent-RL-E{args.epochs}-B{args.batch_size}-LR{args.learning_rate}",
            id=wandb_id,
            resume=resume,
        )
