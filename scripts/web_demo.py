...
def execute_tool(tool_name, args):
    ...
    try:
        if tool_name == "calculate_math":
            ...
            return {
                "result": f"{args.get('value', 0)} {args.get('from_unit', '')} = ? {args.get('to_unit', '')}"
            }
        elif tool_name == "get_current_weather":
            return {"result": f"{args.get('city', 'Unknown')}: 晴, 7~10°C"}
        elif tool_name == "get_exchange_rate":
            return {
                "result": f"1 {args.get('from_currency', 'USD')} = 7.2 {args.get('to_currency', 'CNY')}"
            }
...
def process_assistant_content(content, is_streaming=False):
    ...
    if "<tool_call>" in content:
        ...
        content = re.sub(
            r"<tool_call>(.*?)</tool_call>", format_tool_call, content, flags=re.DOTALL
        )
    ...
    if "<think>" in content and "</think>" in content:
        ...
        content = re.sub(
            r"(<think>)(.*?)(</think>)", format_think, content, flags=re.DOTALL
        )
    ...
    if "<think>" in content and "</think>" not in content:
        ...
        content = re.sub(
            r"<think>(.*?)$", format_think_in_progress, content, flags=re.DOTALL
        )
    ...
    if "<think>" not in content and "</think>" in content:
        ...
        content = re.sub(
            r"(.*?)</think>", format_think_no_start, content, flags=re.DOTALL
        )
...
slogan = (
    f"我是 {MODEL_PATHS[selected_model][1]}，有什么可以帮你的？"
    if st.session_state.get("lang", "en") == "zh"
    else f"I am {MODEL_PATHS[selected_model][1]}, how can I help you?"
)
...
st.session_state.max_new_tokens = st.sidebar.slider(
    get_text("max_length"), 256, 8192, 8192, step=1
)
...
def main():
    ...
    for i, message in enumerate(messages):
        if message["role"] == "assistant":
            st.markdown(
                process_assistant_content(message["content"]), unsafe_allow_html=True
            )
        else:
            st.markdown(
                f'<div style="display: flex; justify-content: flex-end;"><div style="display: inline-block; margin: 10px 0; padding: 8px 12px 8px 12px; background-color: #3d4450; border-radius: 22px; color: white;">{message["content"]}</div></div>',
    ...
    if prompt:
        ...
        sys_prompt = (
            ...
            if tools
                ...
                {
                    "role": "system",
                    "content": "你是MiniMind，一个乐于助人、知识渊博的AI助手。请用完整且友好的方式回答用户问题。",
                }
            ]
        )
        st.session_state.chat_messages = (
        ...
        for new_text in streamer:
            ...
            placeholder.markdown(
                process_assistant_content(answer, is_streaming=True),
                unsafe_allow_html=True,
            )
        ...
        for _ in range(16):
            ...
            for tc_str in tool_calls:
                try:
                    ...
                    tool_results.append(
                        f'<div style="background: rgba(90, 130, 110, 0.20); border: 1px solid rgba(150, 200, 170, 0.30); padding: 10px 12px; border-radius: 12px; margin: 6px 0;"><div style="font-size:12px;opacity:.75;display:block;margin:0 0 6px 0;line-height:1;">ToolCalled</div><div><b>{tc.get("name", "")}</b>: {json.dumps(result, ensure_ascii=False)}</div></div>'
                    )
                except:
                    pass
            full_answer += "\n" + "\n".join(tool_results) + "\n"
            ...
            generation_kwargs["max_length"] = (
                inputs.input_ids.shape[1] + st.session_state.max_new_tokens
            )
