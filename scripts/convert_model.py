...
def convert_torch2transformers_minimind(
    ...
    if int(transformers.__version__.split(".")[0]) >= 5:
        tokenizer_config_path, config_path = (
            os.path.join(transformers_path, "tokenizer_config.json"),
            os.path.join(transformers_path, "config.json"),
        )
        json.dump(
            {
            ...
            open(tokenizer_config_path, "w", encoding="utf-8"),
            indent=2,
            ensure_ascii=False,
        )
...
def convert_torch2transformers(torch_path, transformers_path, dtype=torch.float16):
    ...
    if not lm_config.use_moe:
        qwen_config = Qwen3Config(
            **common_config, use_sliding_window=False, sliding_window=None
        )
        ...
        if int(transformers.__version__.split(".")[0]) >= 5:
            new_sd = {
                ...
                if "experts." not in k or "gate.weight" in k
            }
            for l in range(lm_config.num_hidden_layers):
                ...
                new_sd[f"{p}.gate_up_proj"] = torch.cat(
                    [
                        torch.stack(
                            [
                                state_dict[f"{p}.{e}.gate_proj.weight"]
                                for e in range(lm_config.num_experts)
                            ]
                        ),
                        torch.stack(
                            [
                                state_dict[f"{p}.{e}.up_proj.weight"]
                                for e in range(lm_config.num_experts)
                            ]
                        ),
                    ],
                ...
                new_sd[f"{p}.down_proj"] = torch.stack(
                    [
                        state_dict[f"{p}.{e}.down_proj.weight"]
                        for e in range(lm_config.num_experts)
                    ]
                )
            state_dict = new_sd
...
if __name__ == "__main__":
    lm_config = MiniMindConfig(
        hidden_size=768, num_hidden_layers=8, max_seq_len=8192, use_moe=False
    )
